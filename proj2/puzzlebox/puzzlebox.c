...
void phase01() {
    int a = atoi(next_input());//1+39
    int b = atoi(next_input());//10000
    int c = atoi(next_input());//1000
...
void phase02() {
    int a = atoi(next_input());//1
    int b = atoi(next_input());//1
    int c = atoi(next_input());//224
    if (a == 0) {
        goto FAIL2;
    }
...
void phase03() {
    ...
    int hit = shot ^ targ;//hit=0 short is same as targ
    hit = !hit;
...
void phase04() {
    char *s = next_input();//abcdefgabcdefgh
    int a = atoi(next_input());//0
    int b = atoi(next_input());//6
    int c = atoi(next_input());//13
    int len = strlen(s);//
    ...
    int idx = 0;
    while (idx < len1 && idx < len2) {//len1=len2=7
...
void phase06() {
    ...
    *(((int *)actual) + c) = d;//and 
    ...
    if (strncmp(expect,actual,BUFSIZE) == 0) {
        return;//expect is identical to actual
    }
...
void phase07() {
    ...
    flint.i = hash;//int: 1498700099
    //0b 10110010 10101000 10110010 1000011
    //3.73567746e+15

    if (fabs(a-flint.f) < 1e-8 ) {//a==flint.f
        return;
    }
    char * msg =
...
LCHK:
    if (s == t && m == t) {
        goto LAFT;//need to come here
    }
    s++;
    goto LTOP;
LOC1:
    if (n & 1) {//n is not all zero
        goto LOM1;
    }
    goto LEM1;
LTOP:
    if (!(n > 1)) {//less or equal to 1
        goto LEND;
    }
    goto LOC1;
LEM1:
...
void phase09() {
    int idxs[5] = {};
    int len = strlen(letters);//52
    for (int i = 0; i < 5; i++) {
        idxs[i] = atoi(next_input());//input:idxs
        if (idxs[i] >= len) {
            char *msg =
    ...
    long seed = (((long) hash) << 32) + ~hash;//6436867910218262204
    pb_srand(seed);
    scramble(letters, 100);// letter:0x466c4757
                              

    char buf[BUFSIZE] = {};
    for (int i = 0; i < 5; i++) {
        buf[i] = letters[idxs[i]];//Waldo
    }

    union{
        long l;
        char s[8];//0x646c6157 Waldo

    } chong;
    chong.l = 478426194263;

    if (strcmp(buf, chong.s) == 0) {//buf==s
        return;
    }
    char *msg =
